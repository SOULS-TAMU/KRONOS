    ...
    if (opts.adam_early_exit and opts.use_adam_warmup and not adam_info.skipped
        ...
        if accept.any():
            ...
            runs = [RunResult(theta=X_adam[:, c].copy(), fval=float(fv[c]),
                              converged=bool(accept[c]), iterations=0,
            ...
            return SolveResult(theta=X_adam[:, best].copy(), fval=float(fv[best]),
                               converged=True, runs=runs, best_run=best,
    ...
    if mode == "C":
        ...
        for c in range(X_feas.shape[1]):
            ...
            if opts.verbose and opts.ms_show_runs:
                last = runs[-1]
                tag = (f"Converged in {last.iterations:4d} steps" if last.converged
        ...
        if eligible.any():
            ...
            converged = False
        out = SolveResult(theta=theta, fval=fval, converged=converged, runs=runs,
