...
def solve_cascade(problem: Problem, backend, opts: Options,
    ...
    runsA = resA.runs
    K = len(runsA)
    convA = np.array([r.reformulated_stationary for r in runsA], dtype=bool)
    ...
    for c in range(K):
        ...
        if b is None:
            ...
            picks.append("A")
            continue
        score_a = 3 * a.kkt_certified + a.reformulated_stationary
        score_b = 3 * b.kkt_certified + b.reformulated_stationary
    ...
    conv = np.array([r.reformulated_stationary for r in best_runs], dtype=bool)
    ...
    return SolveResult(
        ...
        reformulated_stationary=bool(conv[best]) if conv.size else False,
